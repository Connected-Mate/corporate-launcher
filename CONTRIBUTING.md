...
- **No surprise behavior** — if a template sets an env var, `references/env-vars.md` documents it. If a flow asks a question, `references/interview-flow.md` lists it.
...
- **Feature request** — the corporate use case, the CLI/backend involved, why the current flow does not cover it. Reference the section of `references/interview-flow.md` you would extend.
...
4. Keep the PR description focused: what changed, why, which `references/` file you updated to match.
...
- `references/` — interview flow, provider matrix, env vars, security patterns. Read this before changing behavior.
