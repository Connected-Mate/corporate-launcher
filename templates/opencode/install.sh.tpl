#!/usr/bin/env bash
# =====================================================================
#  ${CORP_NAME} — installer (opencode wrapper)
...
if command -v opencode >/dev/null 2>&1; then
    info "Underlying CLI present: $(opencode --version 2>/dev/null || echo unknown)"
else
    warn "Underlying CLI not found."
    if ask "Install via npm (npm i -g opencode-ai)?"; then
...
step "[4/7] Install opencode.json"
OPENCODE_DIR="$HOME/.config/opencode"
mkdir -p "$OPENCODE_DIR"
TARGET="$OPENCODE_DIR/opencode.json"

if [ -f "$TARGET" ] && ! grep -qF "Managed by ${CORP_NAME}" "$TARGET" 2>/dev/null; then
    cp "$TARGET" "$\{TARGET\}.${CORP_SLUG}.bak"
    warn "Existing opencode.json backed up at $\{TARGET\}.${CORP_SLUG}.bak"
fi

cp "$INSTALL_DIR/opencode.json" "$TARGET"
chmod 600 "$TARGET"
info "Wrote $TARGET"
...
step "[8/10] Extract corporate CA (if requested)"
...
step "[9/10] Install bundled skills + MCP"
