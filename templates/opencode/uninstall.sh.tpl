#!/usr/bin/env bash
# =====================================================================
#  ${CORP_NAME} — uninstaller (opencode wrapper)
# =====================================================================
...
# tpl: 3. remove opencode.json IF it was ours (marker check)
OPENCODE_JSON="$HOME/.config/opencode/opencode.json"
if [ -f "$OPENCODE_JSON" ] && grep -qF "Managed by ${CORP_NAME}" "$OPENCODE_JSON"; then
    rm -f "$OPENCODE_JSON"
    info "Removed $OPENCODE_JSON"
    # tpl: restore pre-install backup if it exists
    if [ -f "$\{OPENCODE_JSON\}.${CORP_SLUG}.bak" ]; then
        mv "$\{OPENCODE_JSON\}.${CORP_SLUG}.bak" "$OPENCODE_JSON"
        info "Restored previous opencode.json from backup"
    fi
fi

# tpl: 4. remove API token from keychain / file
...
# tpl: 5. remove install dir (last, since we're running from it)
...
printf '  The underlying opencode CLI was not removed (npm uninstall -g opencode-ai if desired).\n'
